...
std::vector<cv::Rect> FDet::detectAll(cv::Mat im,int size){
    vector<cv::Rect> face_detections;
    classifier.detectMultiScale(im, face_detections, 1.1, 3, 0, cv::Size(size, size));
...
//    NSString *location = [[NSBundle mainBundle] resourcePath];
    //    classifier.load([location UTF8String] + std::string("/classifiers/haarcascade_frontalface_alt.xml"));
    classifier.detectMultiScale(im, face_detections, 1.2, 2, 0, cv::Size(120, 120));
    ...
    for( size_t face = 0; face < face_detections.size(); ++face)
    {
        ...
        // Correct for scale
        //        face_detections[face].width = face_detections[face].width * 0.8924;
        //        face_detections[face].height = face_detections[face].height * 0.8676;
        //
        //        // Move the face slightly to the right (as the width was made smaller)
        //        face_detections[face].x = face_detections[face].x + 0.0578 * face_detections[face].width;
        //        // Shift face down as OpenCV Haar Cascade detects the forehead as well, and we're not interested
        //        face_detections[face].y = face_detections[face].y + face_detections[face].height * 0.2166;
...
{  
  ...
  for(i = 0; i < n; i++){
    ...
    NSString *location = [[NSBundle mainBundle] resourcePath];
    classifier = cv::CascadeClassifier([location UTF8String] + std::string("/model/haarcascade_frontalface_alt.xml"));
    return;
}
//===========================================================================
