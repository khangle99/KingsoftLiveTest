...
- (id)initWithSessionPreset:(NSString *)sessionPreset cameraPosition:(AVCaptureDevicePosition)cameraPosition useYuv:(bool)useYuv{
    if (!(self = [super init]))
    {
        return nil;
    }
    cameraProcessingQueue = dispatch_queue_create("session queue", DISPATCH_QUEUE_SERIAL);
//    cameraProcessingQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH,0);
    ...
    if(!useYuv){
        [videoOutput connectionWithMediaType:AVMediaTypeVideo].videoOrientation = AVCaptureVideoOrientationPortrait;
